...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h1 class="text-xl font-bold text-slate-800">Nuevo cliente</h1>
        </template>
        ...
        <div class="mx-auto max-w-xl">
            <form @submit.prevent="submit" class="space-y-5 rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
                <div>
                    ...
                    <TextInput id="nombre" v-model="form.nombre" class="mt-1 block w-full" placeholder="Nombre del cliente" autofocus />
                    <InputError class="mt-2" :message="form.errors.nombre" />
                </div>

                <div>
                    <InputLabel for="telefono" value="Teléfono (WhatsApp)" />
                    <TextInput id="telefono" v-model="form.telefono" class="mt-1 block w-full" placeholder="5215512345678" required />
